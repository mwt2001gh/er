...
//C source codes for the simulation program -- the version corresponds to the case shown in Fig.2c of the paper
...
#define ERCOMSEQ  C,A,G,U,U,C,G,U    // The complement of the characteristic sequence of the ER
#define CTSEQ	  G,U,G,A,U,C,C,A    // The characteristic sequence of a control RNA species
#define CTCOMSEQ  U,G,G,A,U,C,A,C    // The complement of the characteristic sequence of the control species
//***********sequence***************
#define INOCUSEQ ERSEQ,U,A,G        // An inoculated RNA species  
#define INOCUSEQ1 CTSEQ,U,A,G       // Another inoculated RNA species
#define INOCUNUM 10  //cir-er-11nt-denovo: 1       // The molecular number of the inoculated species
#define CUTSTEP 2500000  //2000000    //The step for the cut
...
#define PEL  (i<CUTSTEP?0.000005:0)  // Probability of end-to-end ligation of an RNA molecule (circularization)  
#define PAT  0.5         // Probability of attracting a substrate by a template when the substrate could base-pair with the template
#define PTL  0.05        // Probability of a template-directed ligation 
#define PMNP 0.01		 // Probability of movement of nucleotide precursor
#define PMN  0.005       // Probability of movement of nucleotides
#define PNF  0.05        // Probability of a nucleotide forming from a nucleotide precursor
...
#define FLT  (i<CUTSTEP?0.5:1)    //Fig2b: #define FLT 0.5      // The factor concerning a linear RNA acting as a template
...
void cut(char sq1[], char sq2[], int sqlength); 
...
static char ercomseq[50] = { ERCOMSEQ };
...
float ercom[(STEPNUM - STAREC) / RECINT + 1];
float cir_ercom[(STEPNUM - STAREC) / RECINT + 1];
...
void record(void)               // Data recording
{
	int ch_num[MAX_RNA_LENGTH], ch_c_num[MAX_RNA_LENGTH], ch_er_num[MAX_RNA_LENGTH], ch_0_num[MAX_RNA_LENGTH], long_chain_num, si;//Liang-1 
	...
	for (y = 0; y < N; y++)
	{
		for (x = 0; x < N; x++)
		{
			...
			for (p = room_head[h][y][x]->next; p != room_head[h][y][x]; p = p->next)
			{
				...
				flag = findseq(ercomseq, erlength, p);
				if (flag == 0)
				{
					if (p->type1 == 0) ercom[g]++;
					else cir_ercom[g]++;
				}
	...
	printf("----- step=%d: er=%d-%d|%d (%d|%d), ct=%d|%d (%d|%d), unit=%d (%d)  (tm=%d, np=%d)\n", i,
		(int)er[g], (int)er_rib[g], (int)ercom[g], (int)cir_er[g], (int)cir_ercom[g], (int)ct[g], (int)ctcom[g], (int)cir_ct[g], (int)cir_ctcom[g],
		(int)unit[g], (int)cir_unit[g], (int)total_mat_num[g], (int)np_num[g]);
	fprintf(fptxt, "----- step=%d: er=%d-%d|%d (%d|%d), ct=%d|%d (%d|%d), unit=%d (%d)  (tm=%d, np=%d)\n", i,
		(int)er[g], (int)er_rib[g], (int)ercom[g], (int)cir_er[g], (int)cir_ercom[g], (int)ct[g], (int)ctcom[g], (int)cir_ct[g], (int)cir_ctcom[g],
		(int)unit[g], (int)cir_unit[g], (int)total_mat_num[g], (int)np_num[g]);
	fprintf(fptxt1, "step=%d: er=%d-%d|%d (%d|%d), ct=%d|%d (%d|%d), unit=%d (%d)  (tm=%d, np=%d)\n", i,
		(int)er[g], (int)er_rib[g], (int)ercom[g], (int)cir_er[g], (int)cir_ercom[g], (int)ct[g], (int)ctcom[g], (int)cir_ct[g], (int)cir_ctcom[g],
		(int)unit[g], (int)cir_unit[g], (int)total_mat_num[g], (int)np_num[g]);
	...
	for (y = 0; y < N; y++)
	{
		for (x = 0; x < N; x++)
		{
			for (p = room_head[h][y][x]->next; p != room_head[h][y][x]; p = p->next)
			{
				...
				if (flag1 == 0)ch_er_num[p->length1 - 1]++;
	...
	{
		printf("%dnt-%d(%d)(%d|%d), ", si + 1, ch_num[si], ch_c_num[si], ch_er_num[si], ch_0_num[si]);
		fprintf(fptxt, "%dnt-%d(%d)(%d|%d), ", si + 1, ch_num[si], ch_c_num[si], ch_er_num[si], ch_0_num[si]);
	}
...
//-----------------------------------------------------------------------
void cut(char sq1[], char sq2[], int sqlength)     // Cut circular RNA  to linear RNA with characteristic domain reserved
{
	...
	for (y = 0; y < N; y++)
	{
		for (x = 0; x < N; x++)
		{
			for (p = room_head[h][y][x]->next; p != room_head[h][y][x]; p = p->next)
			{
				if (p->type1 == 1)    // circular chains will be broken
				{
					flag_1 = findseq(sq1, sqlength, p);
					flag_2 = findseq(sq2, sqlength, p);

					if (flag_1 == 0 || flag_2 == 0)  // The RNA containing er or ercom subsequence will be broken at a location reserving the subsequence
					{
						p5 = (struct rna*)malloc(LEN);   // Temporary node for checking the propriate position of chain breaking to save the specific sequence
						if (!p5) { printf("\tsyn form_monomer--memeout\n"); exit(0); }
						memset(p5->information, 0, sizeof(p5->information));
						c2f = (struct c2_frag*)malloc(sizeof(struct c2_frag));
						if (!c2f) { printf("\tinit1--memeout\n"); exit(0); }
						p5->chain2 = c2f;
						c2f->next = p5->chain2;
						...
						for (int a = 0; a < p->length1; a++) // Finding the appropriate location
						{
							memset(p5->information, 0, sizeof(p5->information));
							...
							for (int b = 0; b < p->length1; b++)
							{
								...
								if (a + b < p->length1) {
									p5->information[0][b] = p->information[0][a + b];  // Recording the sequence into the temporary node
									p5->information[1][b] = p->information[1][a + b];
								}
								else if (a + b >= p->length1) {
									p5->information[0][b] = p->information[0][a + b - p->length1];
									p5->information[1][b] = p->information[1][a + b - p->length1];
								}
							}
							p5->length1 = p->length1;   // Recording the lengths into the temporary node
							p5->length2 = p->length2;
							p5->type1 = 0;              // The temporary RNA chain is linear
							p5->type2 = 0;

							if (findseq(sq1, sqlength, p5) == 0 || findseq(sq2, sqlength, p5) == 0)
							{	// If the temporary RNA chain reserves the er or ercom subsequence, breaking at this location (determining the chain2 statue for the new linear chain
								if (p->type2 == 0)  // For the situation of non-double circular chains
								{
									c2f2 = (struct c2_frag*)malloc(sizeof(struct c2_frag));  // Creating a new chain2 fragment
									if (!c2f2) { printf("\tinit1--memeout\n"); exit(0); }
									for (c2f1 = p->chain2->next; c2f1 != p->chain2; c2f1 = c2f1->next)
									{
										if (a > c2f1->start && a < c2f1->start + c2f1->length)   // If the breaking location is within a chain2 fragment
										{
											c2f2->start = a;
											c2f2->length = c2f1->start + c2f1->length - a;
											c2f1->length = c2f1->length - c2f2->length;

											c2f2->next = c2f1->next;
											c2f1->next->prior = c2f2;
											c2f1->next = c2f2;
											c2f2->prior = c2f1;
											break;
										}
									...
									for (c2f1 = p->chain2->next; c2f1 != p->chain2; c2f1 = c2f1->next)
									{															// Resetting the start points of the chain2 fragments
										if (c2f1->start < a) c2f1->start += p->length1 - a;
										else c2f1->start -= a;
									}

									while (1)    // Resetting the head node of the linear chain resulting from the breaking
									{
										c2f1 = p->chain2->prior;
										c2f2 = p->chain2->next;
										if (c2f1 == c2f2)break;
										if (c2f1->start < c2f2->start)
										{
											p->chain2->prior = c2f2;
											p->chain2->next = c2f2->next;
											c2f2->next->prior = p->chain2;
											c2f1->next = c2f2;
											c2f2->prior = c2f1;
											c2f2->next = p->chain2;
										}
								...
								for (int b = 0; b < p->length1; b++)  //  Copying the sequence from the temporary RNA chain
								{
									p->information[0][b] = p5->information[0][b];
									p->information[1][b] = p5->information[1][b];
								}

								break;  // Already breaking at the appropriate location, no longer trying to find the location
							}
						}
						free(p5->chain2);   // Free the temporary node
						free(p5);
					}
					p->type1 = 0;  // Set the type of the new chain as linear
					p->type2 = 0;
				}
			}
		}
...
int main()
{
	...
	for (i = 0; i <= STEPNUM; i++)      // Monte-Carlo cycle
	{
		...
		if (i == CUTSTEP)cut(erseq, ercomseq, erlength); // Cut circular RNAs into linear ones
