...
//C source codes for the simulation program -- the version corresponds to the case shown in Fig.6d of the paper
...
#define NRSEQ	  C,U,G,C,A,U,C,A    // The characteristic sequence of NR (RNA replicase ribozyme)
#define ERREPNRSEQ  ERSEQ,REPSEQ,NRSEQ	//The characteristic sequence of super circle(ERSEQ+REPSEQ+NRSEQ)
#define CTSEQ	  G,U,G,A,U,C,C,A    // The characteristic sequence of a control RNA species
#define BIGCTSEQ  CTSEQ,CTSEQ,CTSEQ 
//***********sequence***************
#define INOCUSEQ ERREPNRSEQ        // An inoculated RNA species  
#define INOCUSEQ1 BIGCTSEQ         // Another inoculated RNA species
...
static char errepnrseq[50] = { ERREPNRSEQ };
static char bigctseq[50] = { BIGCTSEQ };
...
int erlength, replength, nrlength, errepnrlength, bigctlength;
...
float errepnr[(STEPNUM - STAREC) / RECINT + 1];  
float cir_errepnr[(STEPNUM - STAREC) / RECINT + 1];
float bigct[(STEPNUM - STAREC) / RECINT + 1];
float cir_bigct[(STEPNUM - STAREC) / RECINT + 1];
...
void inits(void)         // Initialization of the model system
{
	...
	errepnrlength = 0;
	for (j = 0; errepnrseq[j] != 0; j++)
		errepnrlength++;

	bigctlength = 0;
	for (j = 0; bigctseq[j] != 0; j++)
		bigctlength++;
...
void unit_case(void)      // Action of units (molecules) in the system
{
	...
	for (d = 0; d < ROOMNUM; d++)
	{
		...
		for (p = room_head[h][y][x]->next; p != room_head[h][y][x]; p = p->next)  //Events of RNA (including nucleotides)
		{
			...
			case 4:    // Functioning of ribozymes
				...
				{																// A ribozyme cannot be much longer than its characteristic domain
					flag = findseq(erseq, erlength, p);  // ER functioning
					if (flag == 0)
					{
						...
						for (p3 = p->next; p3 != p; p3 = p3->next)
						{
							...
							if (findseq(repseq, replength, p3) == 0 && p3->length2 == 0 && p3->length1 < 1.5 * replength)
								if (randd() < PRP)continue;  
							if (findseq(nrseq, nrlength, p3) == 0 && p3->length2 == 0 && p3->length1 < 1.5 * nrlength)
								if (randd() < PRP)continue;  
...
void record(void)               // Data recording
{
	...
	for (y = 0; y < N; y++)
	{
		for (x = 0; x < N; x++)
		{
			...
			for (p = room_head[h][y][x]->next; p != room_head[h][y][x]; p = p->next)
			{
				...
				flag3 = findseq(nrseq, nrlength, p);
				...
				else if (flag1 != 0 && flag2 != 0 && flag3 == 0)
				{
					...
					{
						nr[g]++;
						if (p->length1 < 1.5 * replength)nr_rib[g]++;
					}
					else cir_nr[g]++;
				}

				flag = findseq(errepnrseq, errepnrlength, p);
				if (flag == 0 )
				{
					if (p->type1 == 0)errepnr[g]++;
					else cir_errepnr[g]++;
				}

				flag = findseq(bigctseq, bigctlength, p);
				if (flag == 0)
				{
					if (p->type1 == 0) bigct[g]++;
					else cir_bigct[g]++;
				}
			}
		}
	...
	printf("----- step=%d: er=%d-%d(%d), rep=%d-%d(%d), nr=%d-%d(%d), bigct=%d(%d), errepnr=%d(%d), unit=%d (%d)  (tm=%d, np=%d)\n", i,
		(int)er[g], (int)er_rib[g], (int)cir_er[g],
		(int)rep[g], (int)rep_rib[g], (int)cir_rep[g],
		(int)nr[g], (int)nr_rib[g], (int)cir_nr[g],
		(int)bigct[g], (int)cir_bigct[g],
		(int)errepnr[g], (int)cir_errepnr[g],
		(int)unit[g], (int)cir_unit[g], (int)total_mat_num[g], (int)np_num[g]);
	fprintf(fptxt, "----- step=%d: er=%d-%d(%d), rep=%d-%d(%d), nr=%d-%d(%d), bigct=%d(%d), errepnr=%d(%d), unit=%d (%d)  (tm=%d, np=%d)\n", i,
		(int)er[g], (int)er_rib[g], (int)cir_er[g],
		(int)rep[g], (int)rep_rib[g], (int)cir_rep[g],
		(int)nr[g], (int)nr_rib[g], (int)cir_nr[g],
		(int)bigct[g], (int)cir_bigct[g],
		(int)errepnr[g], (int)cir_errepnr[g],
		(int)unit[g], (int)cir_unit[g], (int)total_mat_num[g], (int)np_num[g]);
	fprintf(fptxt1, "step=%d: er=%d-%d(%d), rep=%d-%d(%d), nr=%d-%d(%d), bigct=%d(%d), errepnr=%d(%d), unit=%d (%d)  (tm=%d, np=%d)\n", i,
		(int)er[g], (int)er_rib[g], (int)cir_er[g],
		(int)rep[g], (int)rep_rib[g], (int)cir_rep[g],
		(int)nr[g], (int)nr_rib[g], (int)cir_nr[g],
		(int)bigct[g], (int)cir_bigct[g],
